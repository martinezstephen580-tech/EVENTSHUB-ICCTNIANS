...
class EventCart {
    ...
    async addToCart(eventId) {
        ...
        try {
            ...
            const existingRegs = await this.getUserRegistrations(user.id);
            const alreadyRegistered = existingRegs.some(reg => reg.eventId === eventId);
    ...
    // Helper method to get user registrations
    async getUserRegistrations(userId) {
        try {
            // Try to get all registrations and filter client-side
            const allRegistrations = await window.db.readAll('registrations');
    ...
    async checkout() {
        ...
        // Get all existing registrations for this user first
        const existingRegistrations = await this.getUserRegistrations(user.id);
        const existingEventIds = existingRegistrations.map(reg => reg.eventId);

        for (const item of this.cartItems) {
            try {
                // Check if already registered (using our pre-fetched data)
                if (existingEventIds.includes(item.id)) {
                    this.showNotification(`Already registered for "${item.title}"`, 'warning');
                    failedEvents.push(item.title);
                    failedCount++;
                    continue;
                }
        ...
        if (successCount > 0) {
            // Only remove successfully registered events from cart
            this.cartItems = this.cartItems.filter(item => 
                !existingEventIds.includes(item.id) && 
                !failedEvents.includes(item.title)
            );
            this.saveCart();
            this.updateCartDisplay();
        ...
        if (failedCount > 0) {
            let errorMessage = `Failed to register for ${failedCount} event(s)`;
            if (failedEvents.length > 0) {
                errorMessage += `: ${failedEvents.join(', ')}`;
            }
